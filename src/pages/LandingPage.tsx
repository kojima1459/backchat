...
const Section = ({
  ...
  >
    <div className="flex flex-col gap-2 mb-4">
      ...
      {description && <p className="text-sm text-text-sub leading-relaxed">{description}</p>}
    </div>
      ...
      <header className="sticky top-0 z-20 backdrop-blur bg-bg-soft/90 border-b border-border-light">
        <div className="max-w-4xl mx-auto px-4 py-4 flex items-center justify-between">
          <div className="flex flex-col">
            <span className="text-sm text-text-muted">やること、3つだけ。あとはAIに任せる。</span>
      ...
      <main className="max-w-4xl mx-auto px-4 py-8 space-y-6">
        <section className="rounded-3xl bg-card-white border border-border-light shadow-sm p-6 md:p-8 flex flex-col gap-4">
          <div className="flex flex-col gap-3">
            ...
            <h2 className="text-3xl font-black leading-tight text-text-main">今日3つに絞って、前に進める。</h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <ListItem title="今日3つ" body="Todayは3つまで。決めない地獄を終わらせ、迷いを減らす。" />
            <ListItem title="段取り分解" body="AIボタンで完了条件→素材→ドラフト→清書を提案。逆算が苦手でも前に進む。" />
            <ListItem title="共有メモ" body="共有コードで家族・友人とコメント共有。やることの状況を自然に伝えられる。" />
          </div>
        </section>
        ...
        >
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <ListItem title="今日トグル" body="今日の3つだけが上に固定。Backlogは下に流して脳内の混雑を軽減。" />
            <ListItem title="先送りスコア" body="明日/来週スヌーズと先送り回数のスコアで、後回し癖を見える化。" />
          </div>
        </Section>
        ...
        >
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            <ListItem title="改行で複数追加" body="一気に書いて改行だけで分割。手間なくキャプチャできます。" />
            <ListItem title="自動優先" body="AIのToday3確定を使えば、迷わず今日の3つを埋められます。" />
            <ListItem title="メモ付き" body="各タスクにコメントを残し、共有してもそのまま伝わる。" />
          </div>
        </Section>
        ...
        >
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            <ListItem title="5分着手" body="とりあえず手を動かす導線。最初の摩擦を下げます。" />
            <ListItem title="集中25分" body="区切った集中で「終わらない」を避ける。" />
            <ListItem title="休憩で止まる" body="休憩を促すUI。止まれるのが正義、続ける時は確認ステップ付き。" />
          </div>
        </Section>
        ...
        >
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <ListItem title="軽い共有" body="チャット感覚でコメント。怪しさのない自然なToDo共有。" />
            <ListItem title="必要時だけ共有" body="外部共有は操作した時のみ。通常は端末内に保存されます。" />
          </div>
        </Section>
        ...
        >
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            <ListItem title="Android Chrome" body="メニュー → アプリをインストール" />
          </div>
          <p className="text-sm text-text-sub leading-relaxed">
            使い始めは「インボックスに書く → 今日3つにする → 5分着手」。AIを使うなら設定でAPIキーを保存し、「AI段取り分解」「AI今日3つ確定」を試してみてください。
          </p>
        </Section>
        ...
        <footer className="pb-10 text-center text-sm text-text-sub">
          <div className="flex flex-col gap-2 items-center">
            <div className="flex gap-3">
              <a href="/" className="text-main-deep font-semibold hover:underline">
                アプリに戻る
              </a>
              <a href="/health.html" className="text-text-muted hover:underline">
                /health.html
              </a>
            </div>
